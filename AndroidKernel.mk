...
CRATERQ_CMCC_PROJECT ?= $(shell $(PERL) -e '$$of = "n"; while (<>) { if (/CONFIG_MACH_CRATERQ_CHN_CMCC=y/) { $$of = "y"; break; } } print $$of;' $(KERNEL_CONFIG))
...
HLITE_3GDS_PROJECT ?= $(shell $(PERL) -e '$$of = "n"; while (<>) { if (/CONFIG_MACH_HLITE_EUR_3GDS=y/) { $$of = "y"; break; } } print $$of;' $(KERNEL_CONFIG))
CS03_SGLTE_PROJECT ?= $(shell $(PERL) -e '$$of = "n"; while (<>) { if (/CONFIG_MACH_CS03_SGLTE=y/) { $$of = "y"; break; } } print $$of;' $(KERNEL_CONFIG))
BAFFIN2_SGLTE_PROJECT ?= $(shell $(PERL) -e '$$of = "n"; while (<>) { if (/CONFIG_MACH_BAFFIN2_SGLTE=y/) { $$of = "y"; break; } } print $$of;' $(KERNEL_CONFIG))
...
ifeq "$(CRATERQ_CMCC_PROJECT)" "y"
DTS_FILES = $(wildcard $(TOP)/kernel/arch/arm/boot/dts/msm8226-sec-craterq3g-cmcc*.dts)
else
	ifeq "$(CRATERQ_PROJECT)" "y"
	DTS_FILES = $(wildcard $(TOP)/kernel/arch/arm/boot/dts/msm8226-sec-craterq3g-open*.dts)
	endif
endif
ifeq "$(CT01_PROJECT)" "y"
...
ifeq "$(HLITE_3GDS_PROJECT)" "y"
DTS_FILES = $(wildcard $(TOP)/kernel/arch/arm/boot/dts/msm8226-sec-hlite3gdseur*.dts)
endif
ifeq "$(CS03_SGLTE_PROJECT)" "y"
DTS_FILES = $(wildcard $(TOP)/kernel/arch/arm/boot/dts/msm8926-sec-cs03sgltechn*.dts)
endif
ifeq "$(BAFFIN2_SGLTE_PROJECT)" "y"
DTS_FILES = $(wildcard $(TOP)/kernel/arch/arm/boot/dts/msm8226-sec-baffin2sgltechn*.dts)
endif
