	...
	for (i = 0; i < count; i++) {
		...
		case SENSOR_GPIO:
			if (!strcmp(seq_name, "sensor_gpio_reset"))
				...
				ps[i].seq_val = SENSOR_GPIO_EXT_VIO_POWER;
#if defined(CONFIG_MACH_MILLET3G_EUR) || defined(CONFIG_MACH_CRATERQ_CHN_OPEN) || defined(CONFIG_MACH_CRATERVE_CHN_CTC)
...
=======
...
#if defined(CONFIG_MACH_MILLET3G_EUR)|| defined(CONFIG_MACH_CRATERQ_CHN_OPEN) || defined(CONFIG_MACH_CRATERVE_CHN_CTC) || defined(CONFIG_MACH_HLITE_EUR_3GDS)
	...
	if (of_property_read_bool(of_node, "qcom,gpio-ext-vana-power-vt") == true) {
			rc = of_property_read_u32(of_node, "qcom,gpio-ext-vana-power-vt", &val);
			if (rc < 0) {
				pr_err("%s:%d read qcom,gpio-ext-vana-power-vt	rc %d\n",
					__func__, __LINE__, rc);
				goto ERROR;
			} else if (val >= gpio_array_size) {
				pr_err("%s:%d qcom,gpio-ext-vana-power-vt  invalid %d\n",
					__func__, __LINE__, val);
				goto ERROR;
			}
			gconf->gpio_num_info->gpio_num[SENSOR_GPIO_EXT_VANA_POWER_VT] =
				gpio_array[val];
			CDBG("%s qcom,gpio-ext-vana-power %d\n", __func__,
				gconf->gpio_num_info->gpio_num[SENSOR_GPIO_EXT_VANA_POWER_VT]);
	}
	if (of_property_read_bool(of_node, "qcom,gpio-ext-camio-en-vt") == true) {
			rc = of_property_read_u32(of_node, "qcom,gpio-ext-camio-en-vt", &val);
			if (rc < 0) {
				pr_err("%s:%d read qcom,gpio-ext-camio-en-vt  rc %d\n",
					__func__, __LINE__, rc);
				goto ERROR;
			} else if (val >= gpio_array_size) {
				pr_err("%s:%d qcom,qcom,gpio-ext-camio-en-vt  invalid %d\n",
					__func__, __LINE__, val);
				goto ERROR;
			}
			gconf->gpio_num_info->gpio_num[SENSOR_GPIO_EXT_CAMIO_EN_VT] =
				gpio_array[val];
			CDBG("%s qcom,,qcom,gpio-ext-camio-en-vt %d\n", __func__,
				gconf->gpio_num_info->gpio_num[SENSOR_GPIO_EXT_CAMIO_EN_VT]);
	}
#endif
#endif	
